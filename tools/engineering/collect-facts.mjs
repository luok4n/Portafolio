...
// --- tests -------------------------------------------------------------------------------------
// xUnit runs one case per [Fact] and one per [InlineData] on a [Theory], which is the number the
// test runner reports. Counting [Theory] itself would undercount.
function countTests() {
  const files = filesUnder(join(api, 'Portfolio.Tests'), (name) => name.endsWith('.cs'));
  if (files.length === 0) {
    problems.push('no test files found');
    return 0;
  }
  ...
  for (const file of files) {
    ...
    total += countMatches(text, /\[Fact\]/g);
