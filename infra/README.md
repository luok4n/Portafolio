...
`public/cv/` must contain the redacted CVs. They are generated by `tools/cv/build-cv.mjs`, which
needs a headless browser the image has no business carrying:

```bash
cd src/frontend/portfolio-web && npm run content
```

The Dockerfile checks for them and fails the build if they are missing, rather than shipping a site
whose "Download CV" button 404s — the kind of defect nobody notices until a recruiter clicks it.
